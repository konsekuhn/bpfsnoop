...
#
# writesnoop Trace write() syscalls.
#           For Linux, uses BCC, eBPF. Embedded C.
#
# USAGE: writesnoop [-h] [-T] [-U] [-x] [-p PID] [-t TID]
...
class Writesnoop:
    ...
    def setup_bpf(self):
        ...
        int syscall__trace_entry_write(struct pt_regs *ctx, int fd, void *buf, u64 count)
        {
        """
        ...
        if is_support_kfunc:
            bpf_text += bpf_text_kfunc_header_write.replace('FNNAME', fnname_write)
            bpf_text += bpf_text_kfunc_body
        else:
            bpf_text += bpf_text_kprobe
            bpf_text += bpf_text_kprobe_header_write
        ...
        if not is_support_kfunc:
            self.b.attach_kprobe(event=fnname_write, fn_name="syscall__trace_entry_write")
            self.b.attach_kretprobe(event=fnname_write, fn_name="trace_return")
