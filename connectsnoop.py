...
#
# connectsnoop Trace connect() syscalls.
#           For Linux, uses BCC, eBPF. Embedded C.
#
# USAGE: connectsnoop [-h] [-T] [-U] [-x] [-p PID] [-t TID]
...
int trace_return(struct pt_regs *ctx)
{
    ...
    if (last_event != 0) {
        ...
        if ((tsp - last_event->last_ts) < FILTER_INTERVAL_NS &&
            last_event->last_sport == valp->sport &&
            last_event->last_dport == valp->dport &&
            last_event->last_saddr == valp->saddr &&
            last_event->last_daddr == valp->daddr) {
            infotmp.delete(&id);
            return 0;
        }
    }
    ...
    new_event.last_sport = valp->sport;
    new_event.last_dport = valp->dport;
    new_event.last_saddr = valp->saddr;
    new_event.last_daddr = valp->daddr;
    ...
    data.sport = valp->sport;
    data.dport = valp->dport;
    data.saddr = valp->saddr;
...
"""
        bpf_text_kprobe_header_connect = """
int syscall__trace_entry_connect(struct pt_regs *ctx, int fd, struct sockaddr *addr, int addrlen)
{
"""
    ...
    if (bpf_get_current_comm(&val.comm, sizeof(val.comm)) == 0) {
        ...
        val.sport = 0;  // Source port will be filled in exit
        val.dport = bpf_ntohs(addr_in->sin_port);
        val.saddr = 0;  // Source address will be filled in exit
        val.daddr = bpf_ntohl(addr_in->sin_addr.s_addr);
...
KRETFUNC_PROBE(FNNAME, struct pt_regs *regs, int ret)
{
    ...
    int addrlen = PT_REGS_PARM3(regs);
#else
KRETFUNC_PROBE(FNNAME, int fd, struct sockaddr *addr, int addrlen, int ret)
{
#endif
"""
        bpf_text_kfunc_body = """
    ...
    u16 dport = bpf_ntohs(addr_in.sin_port);
    u32 daddr = bpf_ntohl(addr_in.sin_addr.s_addr);
    ...
    if (last_event != 0) {
        ...
        // Only show if:
        // 1. More than FILTER_INTERVAL seconds has passed since last event
        // 2. Different destination port
        // 3. Different destination address
        if ((tsp - last_event->last_ts) < FILTER_INTERVAL_NS &&
    ...
    data.ts    = tsp / 1000;
    data.uid   = bpf_get_current_uid_gid();
    data.sport = 0;  // Source port will be filled in exit
    data.dport = dport;
    data.saddr = 0;  // Source address will be filled in exit
    data.daddr = daddr;
    data.ret   = ret;
...
"""
        b = BPF(text='')
        fnname_connect = b.get_syscall_prefix().decode() + 'connect'
        filter_interval_ns = self.args.interval * 1000000000
        bpf_text = bpf_text % filter_interval_ns
        is_support_kfunc = BPF.support_kfunc()
        if is_support_kfunc:
            bpf_text += bpf_text_kfunc_header_connect.replace('FNNAME', fnname_connect)
            bpf_text += bpf_text_kfunc_body
        else:
            bpf_text += bpf_text_kprobe
            bpf_text += bpf_text_kprobe_header_connect
        ...
        if not is_support_kfunc:
            self.b.attach_kprobe(event=fnname_connect, fn_name="syscall__trace_entry_connect")
            self.b.attach_kretprobe(event=fnname_connect, fn_name="trace_return")
